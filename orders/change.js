...
var func = function (client, msg) {
    ...
    (0, models_1.find_channel)({
        ...
        var _loop_1 = function (i) {
            client.channels.fetch(channels[i].text_channel, { allowUnknownGuild: true }).then(function (tc) {
                if (tc) {
                    // @ts-ignore
                    tc.setName(new_title, "reason: kikimora order from ".concat(msg.author.username)).then(function (_tc) {
                        if (channels[i].voice_channel) {
                            client.channels.fetch(channels[i].voice_channel, { allowUnknownGuild: true }).then(function (vc) {
                                if (vc) {
                                    // @ts-ignore
